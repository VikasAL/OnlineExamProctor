...
while True:
    ...
    if left_pupil is not None and right_pupil is not None:
        ...
        if 0 < end_nose_x < center_nose_x and 0 < end_nose_y < center_nose_y:
        ...
        elif center_nose_x < end_nose_x < (w*10) and center_nose_y < end_nose_y < (h*10):
            ...
            # cv2.putText(frame, "Abnormal behaviour", (90, 200), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 0, 0), 1)
            data_list.append(['0rd', left_pupil, right_pupil, center_x, center_y, p1, p2])
            count=count+1
            df = pd.DataFrame(data_list, columns=['quadrant','left_pupil','right_pupil','gaze_center_x', 'gaze_center_y', 'nose_end_points',
                                      'gaze_end_points'])
            df.to_csv("myrecorded_data.csv")
        ...
        if cv2.waitKey(1) == 27 or count>=3:
            break
