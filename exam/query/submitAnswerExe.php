...
$selAns = $conn->query("SELECT * FROM exam_answers WHERE exmne_id='$exmne_id' AND exam_id='$exam_id' ");
...
{
	$updLastAns = $conn->query("UPDATE exam_answers SET exans_status='old' WHERE exmne_id='$exmne_id' AND exam_id='$exam_id'  ");
	if($updLastAns)
	{
		foreach ($_REQUEST['answer'] as $key => $value) {
			 $value = $value['correct'];
		  	 $insAns = $conn->query("INSERT INTO exam_answers(exmne_id,exam_id,quest_id,exans_answer) VALUES('$exmne_id','$exam_id','$key','$value')");
		}
		if($insAns)
		{
